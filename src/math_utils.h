...
namespace math_utils {
  ...
  inline __uint128_t mod_pow_wrapper(uint64_t base, uint64_t exp, uint64_t mod) {
  ...
  uint64_t find_generator_balanced(uint64_t p) {
    ...
    std::mt19937_64 rng(std::random_device{}());
    std::uniform_int_distribution<int> method_dist(0, 1); // 0 or 1

    if (method_dist(rng) == 0) {
      std::cout << "Using the random approach to find generator." << std::endl;
      return find_big_generator(factors, p);
    } else {
  ...
  inline bool miller_rabin_test_wrapper(uint64_t n, int k=20) {
    return miller_rabin_test((__uint128_t)n, k);
  }
  ...
  uint64_t find_prime() {
    ...
    do {
      candidate = dist(rng);
    } while (!math_utils::miller_rabin_test_wrapper(candidate, 20));
    return candidate;
  }
  ...
  inline bool is_prime(uint64_t n) {
    return miller_rabin_test_wrapper(n, 20);
  }
}
